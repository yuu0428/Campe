...
vm.runInContext('currentSlideIndex=1;elapsed=0;startedAt=null;updateTimer()',context);
...
vm.runInContext('currentSlideIndex=0;updateReadingTarget()',context);
assert.equal(element('paceStatus').textContent,'30秒遅れ');
vm.runInContext('elapsed=0;updateTimer()',context);
assert.equal(element('paceStatus').textContent,'開始前');
assert.equal(element('paceProgress').value,0);
console.log('PASS: remaining time, deadline boundary, lateness, page change, reset');
